...
class keyframeExtract:
    ...
    def parse_keyframe(self, keyframe_step=None):
        ...
        self.total_frame = cap.get(cv2.CAP_PROP_FRAME_COUNT)  # 视频总共的帧数
        ...
        if self.total_frame != self.shot_indexes[-1] + 1:
            ...
            self.shot_indexes[-1] = int(self.total_frame) - 1
        ...
        for i in self.shot_indexes: # 这个shot_indexes没有第一帧，有最后一帧标号
            try:
                ...
                self.shot_time.append(cap.get(cv2.CAP_PROP_POS_MSEC))
                ...
                if keyframe_step is not None:
                    frame_index_list = self.get_keyframe_index_by_interval(keyframe_step, pre_shot_end_index + 1, curr_shot_end_index)
                else:
                    frame_index_list = self.get_keyframe_index_by_preds(self.preds, pre_shot_end_index + 1, curr_shot_end_index)
                # 获取关键帧
                frame_base64_temp = []
                frame_time_temp = []
                frame_index_temp = []
                for frame_index in frame_index_list:
                    cap.set(cv2.CAP_PROP_POS_FRAMES, frame_index)  # 设置要获取的帧号为当前关键帧
                    success, key_frame = cap.read()  # 获取中间帧
                    if success:
                        RGB_img = Image.fromarray(cv2.cvtColor(key_frame, cv2.COLOR_BGR2RGB))
                        frame_base64_temp.append(PIL_to_base64(RGB_img))
                        frame_time_temp.append(cap.get(cv2.CAP_PROP_POS_MSEC))
                ...
                print(f"video {self.video_path} shot {i} error: {e}")
                import traceback
                traceback.print_exc()
